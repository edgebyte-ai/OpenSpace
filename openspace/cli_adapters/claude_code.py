...
class ClaudeCodeCLIAdapter(BaseCLIAdapter, JSONLSessionMixin):
    name = "claude_code"
    ...
    def parse_execution_output(
        ...
        claude_home = Path(os.environ.get("CLAUDE_CODE_HOME", str(Path.home() / ".claude")))
        events = self.parse_session_jsonl(claude_home / "sessions")
